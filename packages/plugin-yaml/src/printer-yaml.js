/* eslint-disable eslint-comments/no-unlimited-disable -- Reason Below */
/* eslint-disable -- Reason Below */
// @ts-nocheck

/**
 * This is a copy/paste of https://github.com/prettier/prettier/blob/main/src/language-yaml/printer-yaml.js
 *
 * There's no inherently good way to inject into the array/objects and replace their printing methods.
 * When copy/pasting again, do it below this comment, and try not to fix anything. Just inject our method where it goes.
 */

"use strict";

/** @typedef {import("prettier-raw/src/document").Doc} Doc */
...
} = require("prettier-raw/src/document/index.js");
const { isPreviousLineEmpty } = require("prettier-raw/src/common/util.js");
const {
  insertPragma,
  isPragma,
} = require("prettier-raw/src/language-yaml/pragma.js");
const { locStart } = require("prettier-raw/src/language-yaml/loc.js");
const embed = require("prettier-raw/src/language-yaml/embed.js");
const {
  ...
  isNode,
  isInlineNode,
} = require("prettier-raw/src/language-yaml/utils.js");
const preprocess = require("prettier-raw/src/language-yaml/print-preprocess.js");
const {
  alignWithSpaces,
  printNextEmptyLine,
  shouldPrintEndComments,
} = require("prettier-raw/src/language-yaml/print/misc.js");
const {
  printFlowMapping,
  printFlowSequence,
} = require("prettier-raw/src/language-yaml/print/flow-mapping-sequence.js");
const printMappingItem = require("prettier-raw/src/language-yaml/print/mapping-item.js");
const printBlock = require("prettier-raw/src/language-yaml/print/block.js");

function genericPrint(path, options, print, customPrint) {
  ...
  if (hasPrettierIgnore(path)) {
    ...
    parts.push(
      group(customPrint() || printNode(node, parentNode, path, options, print))
    );
  }
...
function printNode(node, parentNode, path, options, print, args) {
  switch (node.type) {
    case "root": {
      const { children } = node;
      const parts = [];
...
module.exports = {
  ...
  print: (path, options, print) =>
    genericPrint(path, options, print, () => undefined),
  massageAstNode: clean,
  insertPragma,
  injectablePrint: genericPrint,
};
